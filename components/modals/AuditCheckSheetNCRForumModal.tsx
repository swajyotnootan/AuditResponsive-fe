...
const AuditCheckSheetNCRForumModal: React.FC<
...
> = ({
  ...
  const getParticipantsList = useCallback(() => {
    ...
    if (auditorId) {
      ...
      if (auditorUser?.email) {
        ...
        addParticipant(
          auditorId,
          auditorName.includes("@") ? auditorName : "",
          auditorName,
          "AUDITOR",
        );
      }
    ...
    if (auditeeId) {
      ...
      if (auditeeUser?.email) {
        addParticipant(
          ...
          auditeeUser.email,
          auditeeUser.name || "",
          auditeeUser.role || "AUDITEE",
        );
      } else if (auditeeName) {
        addParticipant(
          auditeeId,
          auditeeName.includes("@") ? auditeeName : "",
          auditeeName,
          "AUDITEE",
        );
      }
  ...
  const initializeAuditForum = useCallback(async () => {
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to initialize forum",
      );
      setForumReady(true);
    } finally {
      setLoading(false);
