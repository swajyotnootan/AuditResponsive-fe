...
// Check if fileData is valid (not empty and not too small)
const isValidFileData = (fileData?: string): boolean => {
  if (!fileData) return false;
  if (fileData.length < 100) return false; // Too small to be a real image
  return true;
};
...
const WebVideoPlayer = ({
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      videoRef.current.play().catch(() => setError("Cannot play video"));
    }
  };
  ...
  return (
    <View style={styles.videoModalContainer}>
      ...
      <View style={styles.videoModalControls}>
        ...
        <Text style={styles.videoModalStatus}>
...
const AudioPlayer = ({ uri, fileName }: { uri: string; fileName?: string }) => {
  ...
  if (Platform.OS === "web") {
    return <WebAudioPlayer uri={uri} fileName={fileName} />;
  }
...
export default function ThreadCard({
  ...
  const [imageFetchAttempts, setImageFetchAttempts] = useState<
  ...
  // Process attachments - check fileData validity
  const processedAttachments = useMemo(() => {
    ...
    return thread.attachments
      ...
      .map((attachment) => {
        ...
        // Check if fileData is valid (not empty and not corrupted)
        if (attachment.fileData && attachment.fileData.length > 0) {
          // Check if it's a valid base64 string (not too short)
          if (attachment.fileData.length > 100) {
            ...
            console.warn(
              `⚠️ Corrupted data for ${attachment.fileName}: ${attachment.fileData.length} bytes`,
            );
          }
        }

        // If no valid fileData, try cached data
        if (
          !uri &&
          attachment.attachmentType === "IMAGE" &&
          attachment.id &&
          imageDataCache[attachment.id]
        ) {
...
uri = `${API_BASE_URL}/api/forum/8d/files/${attachment.id}`;        }
        ...
        return {
          ...attachment,
          uri,
          hasValidFileData: isValidData,
        };
      });
  }, [thread.attachments, imageDataCache]);

  // Load image data from server - ONLY if no valid fileData exists
  const loadImageData = async (
    attachment: Attachment & { hasValidFileData?: boolean },
  ) => {
    if (!attachment.id) return;
    if (attachment.attachmentType !== "IMAGE") return;
    if (loadingImages[attachment.id]) return;
    if (imageDataCache[attachment.id]) return;
    if (imageErrors[attachment.id]) return;

    // CRITICAL FIX: If we have valid fileData, DON'T fetch from server
    if (attachment.hasValidFileData) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `❌ Server returned ${response.status} for ${attachment.fileName}`,
        );
        setImageErrors((prev) => ({ ...prev, [attachment.id!]: true }));
      ...
      // Check if blob is valid (must be > 100 bytes for a real image)
      if (blob.size < 100) {
        console.warn(
          `❌ Image too small (${blob.size} bytes): ${attachment.fileName}`,
        );
        setImageErrors((prev) => ({ ...prev, [attachment.id!]: true }));
        setLoadingImages((prev) => ({ ...prev, [attachment.id!]: false }));
        return;
      ...
      const dataUri = await new Promise<string>((resolve, reject) => {
        reader.onloadend = () => {
          ...
          let dataUriResult = result;
          if (!result.startsWith("data:")) {
            dataUriResult = `data:${contentType};base64,${result}`;
          }
      ...
      console.log(
        `✅ Image fetched successfully: ${attachment.fileName} (${blob.size} bytes)`,
      );
    } catch (error) {
      console.warn(`❌ Failed to fetch image ${attachment.fileName}:`, error);
      setImageErrors((prev) => ({ ...prev, [attachment.id!]: true }));
    } finally {
      setLoadingImages((prev) => ({ ...prev, [attachment.id!]: false }));
    }
  ...
  const downloadFile = async (attachment: Attachment) => {
    try {
      ...
      if (
        ...
        openPdfPreview(url, attachment.fileName || "document.pdf");
        return;
      }
  ...
  return (
    <>
      ...
      <Modal visible={pdfModal.open} onRequestClose={closePdfPreview}>
        <PDFViewerModal url={pdfModal.url} onClose={closePdfPreview} />
      </Modal>
        ...
        >
          {!isOwnMessage && (
            <Text style={styles.senderName}>
              {thread.createdByName || thread.createdBy || "User"}
            </Text>
          )}
          ...
          {thread.content && thread.messageType !== "EVENT" && (
            <Text style={styles.messageText}>{thread.content}</Text>
          )}
