...
const getStyles = (isMobile: boolean, isTablet: boolean) =>
  StyleSheet.create({
    ...
    // ... inside getStyles function ...

    // 1. Fixed Header Layout (prevents wrapping/overlap on mobile)
    header: {
      flexDirection: "row",
    ...
    headerLeft: {
      ...
      alignItems: "center",
      marginRight: 12,
      flexShrink: 1, // Changed from flex: 1 to flexShrink: 1 so it doesn't force the button down
    },
    ...
    headerTitle: {
      fontSize: isMobile ? 18 : 26, // Slightly smaller on mobile to prevent overflow
      fontWeight: "800",
      color: "#1F2937",
    },
    ...
    // ... (keep other styles the same until tableHeader) ...

    // 2. Fixed Table Header Visibility (better contrast and padding on mobile)
    tableHeader: {
      flexDirection: "row",
    ...
    headerCell: {
      fontSize: isMobile ? 12 : 13, // Increased from 11 to 12 for readability
      fontWeight: "700",
      color: "#1F2937", // Darkened from '#4B5563' for better contrast
      textAlign: "center",
      paddingHorizontal: isMobile ? 8 : 4, // Increased horizontal padding on mobile
    ...
    modalHeader: {
      ...
      borderBottomWidth: 1,
      borderBottomColor: "#E5E7EB",
      backgroundColor: "#F9FAFB", // Added background
    },

    modalTitle: {
      fontSize: isMobile ? 16 : 20, // Smaller on mobile
      fontWeight: "700",
      color: "#1F2937",
      flex: 1,
    },
...
const HRAdminDashboard: React.FC = () => {
  ...
  const fetchAuditors = async () => {
    try {
      ...
      const auditorList = allUsers
        .filter((u: any) =>
          ["AUDITOR", "LEAD_AUDITOR"].includes(
            u?.role || u?.userRole || u?.Role || "",
          ),
        )
        .map((u: any) => ({
          id: u.id || u._id,
          ...
          internalAuditorTraining:
            u.internalAuditorTraining || u.internal_auditor_training || "",
          coreToolsTraining: u.coreToolsTraining || u.core_tools_training || "",
          customerSpecificApproved: u.customerSpecificApproved || false,
          problemSolvingTools:
            u.problemSolvingTools || u.problem_solving_tools || "",
          certifiedForProcess:
            u.certifiedForProcess || u.certified_for_process || "",
          certifiedForProduct:
            u.certifiedForProduct || u.certified_for_product || "",
          certificationDate: u.certificationDate || u.certification_date || "",
          certificationExpiryDate:
            u.certificationExpiryDate || u.certification_expiry_date || "",
      ...
      const avgExp =
        auditorList.length > 0
          ? auditorList.reduce((sum, a) => sum + (a.totalExperience || 0), 0) /
            auditorList.length
          : 0;
  ...
  const handleUpdateCompetency = async (
    ...
    try {
      ...
      await userAPI.update(String(auditorId), {
        ...currentUser,
        ...competencyData,
      });
  ...
  const Form1View: React.FC = () => {
    ...
    useEffect(() => {
      const fetchForm1 = async () => {
        try {
          const response = await userAPI.getAll();
          let allUsers = Array.isArray(response)
            ? response
            : (response as any)?.data || [];
          setForm1Auditors(
            allUsers
              .filter((u: any) =>
                ["AUDITOR", "LEAD_AUDITOR"].includes(u?.role || ""),
              )
              .map((u: any) => ({
                id: u.id,
                ...
                internalAuditorTraining: u.internalAuditorTraining,
                certifiedForProcess: u.certifiedForProcess,
                certifiedForProduct: u.certifiedForProduct,
                role: u.role,
              })),
          );
        } catch (e) {
    ...
    return (
      <Modal
        ...
        animationType="slide"
        transparent={true}
        onRequestClose={() => setShowForm1(false)}
      >
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                    <View>
                      <View style={styles.form1Header}>
                        <Text
                          style={[styles.form1HeaderCell, styles.form1SrNoCell]}
                        >
                          Sr NO.
                        </Text>
                        <Text
                          style={[styles.form1HeaderCell, styles.form1NameCell]}
                        >
                          Name
                        </Text>
                        <Text
                          style={[
                            styles.form1HeaderCell,
                            styles.form1SystemCell,
                          ]}
                        >
                          System
                        </Text>
                      ...
                      {form1Auditors.map((item, index) => (
                        <View
                          ...
                          style={styles.form1Row}
                        >
                          <Text
                            style={[styles.form1Cell, styles.form1SrNoCell]}
                          >
                            {index + 1}
                          </Text>
                          <Text
                            style={[styles.form1Cell, styles.form1NameCell]}
                            numberOfLines={1}
                          >
                            {item?.namePrefix} {item?.firstName}{" "}
                            {item?.lastName}
                          </Text>
                          <Text
                            style={[styles.form1Cell, styles.form1SystemCell]}
                            numberOfLines={1}
                          >
                            {item?.internalAuditorTraining || "-"}
                          </Text>
                          <Text
                            style={[styles.form1Cell, styles.form1ProcessCell]}
                            numberOfLines={1}
                          >
                            {truncateList(
                              item?.certifiedForProcess,
                              isMobile ? 1 : 2,
                            )}
                          </Text>
                          <Text
                            style={[styles.form1Cell, styles.form1ProductCell]}
                            numberOfLines={1}
                          >
                            {truncateList(
                              item?.certifiedForProduct,
                              isMobile ? 1 : 2,
                            )}
                          </Text>
                        </View>
  ...
  // Competency Form
  const CompetencyForm: React.FC<{
    auditor: User;
    onSave: (data: any) => void;
    onCancel: () => void;
  }> = ({ auditor, onSave, onCancel }) => {
    const [formData, setFormData] = useState<CompetencyFormData>({
      ...
      certificationExpiryDate:
        auditor?.certificationExpiryDate?.split("T")[0] || "",
    });

    const handleProcessToggle = (p: string) =>
      setFormData((prev) => ({
        ...prev,
    ...
    const handleToolToggle = (
      tool: string,
      field: "coreToolsTraining" | "problemSolvingTools",
    ) => {
      const current = formData[field] ? formData[field].split(",") : [];
      setFormData((prev) => ({
        ...prev,
        [field]: (current.includes(tool)
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                Competency: {auditor?.firstName} {auditor?.lastName}
              </Text>
            ...
            >
              ...
              <View style={styles.infoGrid}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>{auditor?.email}</Text>
                </View>
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Department</Text>
                  <Text style={styles.infoValue}>
                    {getDepartmentName(auditor?.department)}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>Competency Details</Text>
              </View>
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(t) =>
                    setFormData({ ...formData, qualification: t })
                  }
                />
              </View>
              <View style={styles.formGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(t) =>
                    setFormData({ ...formData, totalExperience: t })
                  }
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(t) =>
                    setFormData({ ...formData, internalAuditorTraining: t })
                  }
                />
              </View>
                ...
                <View style={styles.toolContainer}>
                  {coreToolsList.map((tool) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleToolToggle(tool, "coreToolsTraining")
                      }
                    >
                      <Text
                        style={[
                  ...
                  {processes.map((p) => (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        formData.certifiedForProcess.includes(p) &&
                          styles.processButtonActive,
                      ]}
                      onPress={() => handleProcessToggle(p)}
                    >
                      <Text
                        style={[
                          styles.processButtonText,
                          formData.certifiedForProcess.includes(p) &&
                            styles.processButtonTextActive,
                        ]}
                      >
                        {p}
                      </Text>
              ...
              <View style={styles.switchContainer}>
                ...
                <Switch
                  value={formData.customerSpecificApproved}
                  onValueChange={(v) =>
                    setFormData({ ...formData, customerSpecificApproved: v })
                  }
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <View style={styles.headerIcon}>
              <Icon name="award" size={24} color="#6C63FF" />
            </View>

            {/* Added this wrapper View to handle text shrinking gracefully */}
            <View style={{ flexShrink: 1 }}>
              <Text style={styles.headerTitle} numberOfLines={1}>
                HR Dashboard
              </Text>
              <Text style={styles.headerSubtitle} numberOfLines={1}>
                Manage auditor competencies
              </Text>
            </View>
          </View>
          ...
          >
            <View>
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.headerCell, styles.trainingCell]}>
                  Training
                </Text>
                {coreToolsList.map((tool) => (
                  <Text
                    key={tool}
                    style={[styles.headerCell, styles.coreToolCell]}
                  >
                    {tool}
                  </Text>
                ))}
                <Text style={[styles.headerCell, styles.processCell]}>
                  Process
                </Text>
                <Text style={[styles.headerCell, styles.productCell]}>
                  Product
                </Text>
                <Text style={[styles.headerCell, styles.solvingCell]}>
                  Solving
                </Text>
              ...
              {filteredAuditors.length > 0 ? (
                filteredAuditors.map((item, index) => (
                  <View key={String(item.id || index)} style={styles.tableRow}>
                    ...
                    <Text
                      style={[styles.tableCell, styles.qualCell]}
                      numberOfLines={1}
                    >
                      {item.qualification || "-"}
                    </Text>
                    <Text style={[styles.tableCell, styles.expCell]}>
                      {item.totalExperience || 0}
                    </Text>
                    <Text
                      style={[styles.tableCell, styles.trainingCell]}
                      numberOfLines={1}
                    >
                      {item.internalAuditorTraining ? (
                    ...
                    {coreToolsList.map((tool) => (
                      <Text
                        key={tool}
                        style={[styles.tableCell, styles.coreToolCell]}
                      >
                        {item.coreToolsTraining?.includes(tool) ? "✓" : "-"}
                      </Text>
                    ))}

                    <Text
                      style={[styles.tableCell, styles.processCell]}
                      numberOfLines={1}
                    >
                    ...
                    <Text
                      style={[styles.tableCell, styles.solvingCell]}
                      numberOfLines={1}
                    >
                    ...
                    <TouchableOpacity
                      style={[styles.actionButton, styles.actionCell]}
                      onPress={() => {
                        setEditingAuditor(item);
                ...
                <View style={styles.emptyContainer}>
                  <Text style={styles.emptyText}>
                    {searchTerm ? "No matches found" : "No auditors available"}
                  </Text>
                </View>
              )}
            </View>
