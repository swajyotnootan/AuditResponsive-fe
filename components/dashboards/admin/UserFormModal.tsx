...
import {
  ...
  Upload,
  X,
} from "lucide-react-native"; // ✅ Added Search
...
const StyledInput = React.memo(
  ({ value, onChangeText, placeholder, ...props }: any) => (
    <TextInput
      style={{
        borderWidth: 1,
        borderColor: "#d1d5db",
...
const PickerModal = React.memo(
  ({ visible, onClose, title, options, selected, onSelect }: any) => {
                  ...
                  {MONTHS.map((month, idx) => (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        backgroundColor:
                          selectedMonth === idx ? "#1e3a5f" : "transparent",
                        marginBottom: 4,
                      }}
                    >
                      <Text
                    ...
                    >
                      <Text
                        style={{
                          fontSize: 14,
                          color:
                            selectedDay === Number(day) ? "white" : "#374151",
                          fontWeight:
                            selectedDay === Number(day) ? "600" : "400",
                          textAlign: "center",
                        }}
                      >
                        {day}
                  ...
                  {years.map((year) => (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        backgroundColor:
                          selectedYear === year ? "#1e3a5f" : "transparent",
                        marginBottom: 4,
                      }}
                    >
                      <Text
...
// ✅ Add this helper function to parse combined phone numbers
const parseInitialPhone = (combinedPhone: string | undefined) => {
  if (!combinedPhone)
    return { phone: "", countryCode: "+91", countryIsoCode: "IN" };

  try {
    // Try parsing with libphonenumber-js (handles "+919876543210" perfectly)
    ...
    // Fallback regex: matches +[1-4 digits] followed by [4-15 digits]
  ...
  // If no country code is found (e.g., backend only saved "9876543210"), assume defaults
  return {
...
export default function UserFormModal({
  ...
  const initialPhoneData = parseInitialPhone(user?.phone);
  const [phone, setPhone] = useState(initialPhoneData.phone);
  const [countryCode, setCountryCode] = useState(
    user?.countryCode || initialPhoneData.countryCode,
  );
  const [countryIsoCode, setCountryIsoCode] = useState(
    user?.countryIsoCode || initialPhoneData.countryIsoCode,
  );
  const [formData, setFormData] = useState({
    ...
    certifiedForProcess: Array.isArray(user?.certifiedForProcess)
      ? user.certifiedForProcess
      : user?.certifiedForProcess
        ? user.certifiedForProcess.split(",")
        : [],
  ...
  const isAuditorRole =
    formData.role === "AUDITOR" || formData.role === "LEAD_AUDITOR";
    ...
    try {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/api/enterprise/plants/company/${companyId}`,
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        data = await res.json();
      }
  ...
  const fetchSites = async (plantId: string) => {
    try {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/api/enterprise/sites/plant/${plantId}`,
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        data = await res.json();
      }
  ...
  const fetchUnits = async (siteId: string) => {
    try {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/api/enterprise/units/site/${siteId}`,
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        data = await res.json();
      }
  ...
  const handleSave = useCallback(() => {
    ...
    const combinedPhone = phone ? `${countryCode}${phone}` : "";

    onSave({
      ...formData,
      phone: combinedPhone, // ✅ Use the phone state
  ...
  const takePhoto = useCallback(async () => {
    if (Platform.OS === "web") {
      Alert.alert(
        "Web",
        "Camera not available on web. Use Choose Photo instead.",
      );
      return;
    }
    ...
    <Modal visible transparent animationType="slide">
      ...
      >
        ...
        >
          ...
          >
            <TouchableOpacity onPress={() => confirmDiscard(onClose)}>
            ...
            {["Basic", "Work", "Competency"].map((s, i) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  marginHorizontal: 2,
                  borderRadius: 4,
                  backgroundColor: step === i + 1 ? "#1e3a5f" : "#e5e7eb",
                }}
              >
                <Text
                  style={{
              ...
              <>
                  ...
                  <View style={{ flex: 1 }}>
                    <Field label="Last Name" required>
                      <StyledInput
                        ...
                        onChangeText={(t: string) => updateField("lastName", t)}
                ...
                <Field label="Username">
                  <StyledInput
                    ...
                    onChangeText={(t: string) => updateField("userName", t)}
                ...
                <Field label="Phone Number">
                  ...
                  {phoneError ? (
                    <Text
                      style={{ marginTop: 4, fontSize: 12, color: "#ef4444" }}
                    >
                      {phoneError}
                    </Text>
                  ) : null}
                ...
                <Field label="Role" required>
                  <PickerButton
                    value={
                      availableRoles.find((r) => r.name === formData.role)
                        ?.displayName
                    }
                ...
                >
                  ...
                  <View style={{ gap: 8 }}>
                    ...
                    >
                      ...
                      <Text
                        style={{ color: "white", fontSize: 12, marginLeft: 6 }}
                      >
                        Choose Photo
                      </Text>
                    </TouchableOpacity>
              ...
              <>
                ...
                <Field label="Employee ID">
                  <StyledInput
                    ...
                    onChangeText={(t: string) => updateField("employeeId", t)}
                    placeholder="EMP001"
                  />
                </Field>
                <Field label="Designation">
                  <StyledInput
                    ...
                    onChangeText={(t: string) => updateField("designation", t)}
                    ...
                    onPress={() => {
                      Alert.alert("Select", "", [
                        ...EMPLOYMENT_TYPES.map((t) => ({
                          text: t,
                          onPress: () => updateField("employmentType", t),
                        })),
                        { text: "Cancel", onPress: () => {} },
                      ]);
                    }}
                  ...
                  <View style={{ flex: 1 }}>
                    <Field label="Confirmation Date">
                      <DateInput
                        ...
                        onPress={() => setShowDatePicker("confirmationDate")}
                        placeholder="Select date"
                      />
                    </Field>
                  </View>
                ...
                <Field label="City">
                  <StyledInput
                    ...
                    onChangeText={(t: string) => updateField("workCity", t)}
                    placeholder="City"
                  />
                </Field>
                <View style={{ flexDirection: "row", gap: 8 }}>
                ...
                <Field label="Pincode">
                  <StyledInput
                    ...
                    onChangeText={(t: string) => updateField("workPincode", t)}
                    placeholder="Pincode"
                ...
                <Field label="Company">
                  <PickerButton
                    value={
                      companies.find((c) => String(c.id) === formData.companyId)
                        ?.name || ""
                    }
                ...
                <Field label="Plant">
                  <PickerButton
                    value={
                      plants.find((p) => String(p.id) === formData.plantId)
                        ?.name || ""
                    }
                ...
                <Field label="Site">
                  <PickerButton
                    value={
                      sites.find((s) => String(s.id) === formData.siteId)
                        ?.name || ""
                    }
                ...
                <Field label="Unit">
                  <PickerButton
                    value={
                      units.find((u) => String(u.id) === formData.unitId)
                        ?.name || ""
                    }
                  ...
                  <View style={{ flex: 1 }}>
                    <Field label="HOD ID">
                      <StyledInput
                        ...
                        onChangeText={(t: string) => updateField("hodId", t)}
                        placeholder="HOD ID"
                        keyboardType="numeric"
                      />
                    </Field>
              ...
              <>
                {isAuditorRole ? (
                  <>
                    ...
                    <Field label="Total Experience (Years)">
                      <StyledInput
                        ...
                        onChangeText={(t: string) =>
                          updateField("totalExperience", t)
                        }
                        placeholder="Years"
                        keyboardType="numeric"
                      />
                    </Field>
                    <Field label="Internal Auditor Training">
                      <StyledInput
                        value={formData.internalAuditorTraining}
                        onChangeText={(t: string) =>
                          updateField("internalAuditorTraining", t)
                        }
                    ...
                    >
                      {CORE_TOOLS.map((tool) => {
                        const selected = (formData.coreToolsTraining || "")
                          .split(",")
                          .includes(tool);
                        return (
                          <TouchableOpacity
                            key={tool}
                            onPress={() =>
                              toggleArray("coreToolsTraining", tool)
                            }
                            style={{
                              paddingHorizontal: 12,
                              paddingVertical: 6,
                    ...
                    >
                      {PROBLEM_SOLVING_TOOLS.map((tool) => {
                        const selected = (formData.problemSolvingTools || "")
                          .split(",")
                          .includes(tool);
                        return (
                          <TouchableOpacity
                            key={tool}
                            onPress={() =>
                              toggleArray("problemSolvingTools", tool)
                            }
                            style={{
                              paddingHorizontal: 12,
                              paddingVertical: 6,
                    ...
                    >
                      {PROCESSES.map((process) => {
                        const selected = (
                          formData.certifiedForProcess || []
                        ).includes(process);
                        return (
                          <TouchableOpacity
                            key={process}
                            onPress={() =>
                              toggleArray("certifiedForProcess", process, true)
                            }
                            style={{
                              paddingHorizontal: 12,
                              paddingVertical: 6,
                    ...
                    <Field label="Certified Products">
                      <StyledInput
                        value={formData.certifiedForProduct}
                        onChangeText={(t: string) =>
                          updateField("certifiedForProduct", t)
                        }
                      ...
                      <View style={{ flex: 1 }}>
                        <Field label="Certification Date">
                          <DateInput
                            ...
                            onPress={() =>
                              setShowDatePicker("certificationDate")
                            }
                            placeholder="Select date"
                          />
                        </Field>
                          ...
                          <DateInput
                            value={
                              formData.certificationExpiryDate
                                ? formatDate(formData.certificationExpiryDate)
                                : ""
                            }
                            onPress={() =>
                              setShowDatePicker("certificationExpiryDate")
                            }
                            placeholder="Select date"
                          />
                        </Field>
                  ...
                  <View style={{ alignItems: "center", paddingVertical: 48 }}>
                    ...
                    >
                      Competency settings only for Auditor and Lead Auditor
                      roles
                    </Text>
                  </View>
                )}
      ...
      <DatePickerModal
        ...
        value={
          showDatePicker
            ? (formData[showDatePicker as keyof typeof formData] as string) ||
              ""
            : ""
        }
        onSelect={(dateStr: string) => {
          if (showDatePicker) updateField(showDatePicker, dateStr);
      ...
      <PickerModal
        ...
        onSelect={(v: string) => updateField("department", v)}
      />
      <PickerModal
        visible={showPrefixPicker}
        onClose={() => setShowPrefixPicker(false)}
        title="Select Prefix"
        options={NAME_PREFIXES}
        selected={formData.namePrefix}
        onSelect={(v: string) => updateField("namePrefix", v)}
      />
      <PickerModal
