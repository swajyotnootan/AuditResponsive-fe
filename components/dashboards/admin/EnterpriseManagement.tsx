...
export default function EnterpriseManagement() {
  ...
  const [formData, setFormData] = useState({ name: '', code: '', city: '', state: '', country: '', classification: '', unitType: '' });

  useEffect(() => { loadData(); }, []);

  const loadData = async () => {
    setLoading(true);
    try { const data = await enterpriseAPI.getCompanies(); setCompanies(Array.isArray(data) ? data : []); } 
  ...
  const loadPlants = async (companyId: string) => {
    setLoading(true);
    try { const data = await enterpriseAPI.getPlantsByCompany(companyId); setPlants(Array.isArray(data) ? data : []); } 
    catch (err) { console.error(err); }
    finally { setLoading(false); }
  };
  ...
  const loadSites = async (plantId: string) => {
    setLoading(true);
    try { const data = await enterpriseAPI.getSitesByPlant(plantId); setSites(Array.isArray(data) ? data : []); } 
    catch (err) { console.error(err); }
    finally { setLoading(false); }
  };
  ...
  const loadUnits = async (siteId: string) => {
    setLoading(true);
    try { const data = await enterpriseAPI.getUnitsBySite(siteId); setUnits(Array.isArray(data) ? data : []); } 
    catch (err) { console.error(err); }
    finally { setLoading(false); }
  };

  const getCurrentData = () => {
    switch (view) { case 'companies': return companies; case 'plants': return plants; case 'sites': return sites; case 'units': return units; }
  };
  ...
  const getViewIcon = () => {
    switch (view) { case 'companies': return Building; case 'plants': return Factory; case 'sites': return MapPin; case 'units': return Layers; }
  };

  const handleAdd = () => {
    setEditingItem(null);
    setFormData({ name: '', code: '', city: '', state: '', country: '', classification: '', unitType: '' });
    setShowModal(true);
  };

  const handleEdit = (item: any) => {
    setEditingItem(item);
    setFormData({ name: item.name || '', code: item.code || '', city: item.city || '', state: item.state || '', country: item.country || '', classification: item.classification || '', unitType: item.unitType || '' });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const data = { ...formData };
      if (view !== 'companies') { if (view === 'plants') (data as any).companyId = selectedParent?.id; else if (view === 'sites') (data as any).plantId = selectedParent?.id; else if (view === 'units') (data as any).siteId = selectedParent?.id; }
      if (editingItem) {
        switch (view) { case 'companies': await enterpriseAPI.updateCompany(editingItem.id, data); break; case 'plants': await enterpriseAPI.updatePlant(editingItem.id, data); break; case 'sites': await enterpriseAPI.updateSite(editingItem.id, data); break; case 'units': await enterpriseAPI.updateUnit(editingItem.id, data); break; }
      } else {
        switch (view) { case 'companies': await enterpriseAPI.createCompany(data); break; case 'plants': await enterpriseAPI.createPlant(data); break; case 'sites': await enterpriseAPI.createSite(data); break; case 'units': await enterpriseAPI.createUnit(data); break; }
      }
  ...
  const executeDelete = async (id: string) => {
    ...
    try {
      let url = '';
      switch (view) { case 'companies': url = `${API_BASE_URL}/api/enterprise/companies/${id}`; break; case 'plants': url = `${API_BASE_URL}/api/enterprise/plants/${id}`; break; case 'sites': url = `${API_BASE_URL}/api/enterprise/sites/${id}`; break; case 'units': url = `${API_BASE_URL}/api/enterprise/units/${id}`; break; }
      const response = await fetch(url, { method: 'DELETE' });
      if (response.ok || response.status === 204) { setSuccess('Deleted successfully!'); refreshView(); } 
      else { const errorText = await response.text(); setError(`Delete failed (${response.status})`); }
      setTimeout(() => setSuccess(''), 3000);
  ...
  const refreshView = () => {
    switch (view) { case 'companies': loadData(); break; case 'plants': if (selectedParent) loadPlants(String(selectedParent.id)); break; case 'sites': if (selectedParent) loadSites(String(selectedParent.id)); break; case 'units': if (selectedParent) loadUnits(String(selectedParent.id)); break; }
  };

  const drillDown = (item: any) => {
    setSelectedParent(item);
    switch (view) { case 'companies': loadPlants(String(item.id)); setView('plants'); break; case 'plants': loadSites(String(item.id)); setView('sites'); break; case 'sites': loadUnits(String(item.id)); setView('units'); break; }
  };

  const goBack = () => {
    switch (view) { case 'plants': setView('companies'); break; case 'sites': setView('plants'); break; case 'units': setView('sites'); break; }
  };

  const data = getCurrentData() || [];
  const filteredData = data.filter((item: any) => item.name?.toLowerCase().includes(searchQuery.toLowerCase()) || item.code?.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="px-4 py-3" style={isDesktop ? { maxWidth: 1200, alignSelf: 'center', width: '100%' } : undefined}>
        <View className="flex-row items-center bg-gray-100 rounded-lg px-3 py-2">
          ...
          <TextInput className="flex-1 ml-2 text-sm py-1" placeholder={`Search ${getViewTitle().toLowerCase()}...`} value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor="#9ca3af" />
        </View>
      </View>
                ...
                <View key={item.id} className="bg-white rounded-xl mb-3 shadow-sm border border-gray-100 overflow-hidden">
                  <TouchableOpacity className="px-4 py-3" onPress={() => view !== 'units' ? drillDown(item) : null} disabled={view === 'units'}>
                    <View className="flex-row items-center">
                      ...
                      <View className="flex-row items-center">
                        <TouchableOpacity onPress={() => handleEdit(item)} className="p-2 bg-blue-50 rounded-lg mr-1">
                          <Edit size={14} color="#00529B" />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setDeleteConfirm({ id: String(item.id), name: item.name })} className="p-2 bg-red-50 rounded-lg">
                  ...
                  {filteredData.map((item: any) => (
                    <View key={item.id} className="flex-row items-center py-3 px-4 border-b border-gray-100">
                      ...
                      <View className="w-24 flex-row justify-center gap-1">
                        <TouchableOpacity onPress={() => handleEdit(item)} className="p-1.5 bg-blue-50 rounded-lg"><Edit size={14} color="#00529B" /></TouchableOpacity>
                        <TouchableOpacity onPress={() => setDeleteConfirm({ id: String(item.id), name: item.name })} className="p-1.5 bg-red-50 rounded-lg"><Trash2 size={14} color="#ef4444" /></TouchableOpacity>
                      </View>
                    </View>
                  ))}
                </View>
        ...
        <View className="flex-1 bg-black/50" style={isDesktop ? { justifyContent: 'center', alignItems: 'center' } : { justifyContent: 'flex-end' }}>
          <View style={isDesktop ? { backgroundColor: 'white', borderRadius: 16, width: 500, maxHeight: '80%' } : { backgroundColor: 'white', borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: '80%' }}>
            ...
            <ScrollView className="px-5 py-4" showsVerticalScrollIndicator={false}>
              ...
              <TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.name} onChangeText={(t) => setFormData({...formData, name: t})} placeholder="Enter name" />
              <Text className="text-sm font-medium text-gray-700 mb-1">Code</Text>
              <TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.code} onChangeText={(t) => setFormData({...formData, code: t})} placeholder="Enter code" />
              {view === 'plants' && (
                <View style={isDesktop ? { flexDirection: 'row', gap: 12 } : {}}>
                  <View style={{ flex: 1 }}><Text className="text-sm font-medium text-gray-700 mb-1">City</Text><TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.city} onChangeText={(t) => setFormData({...formData, city: t})} placeholder="City" /></View>
                  <View style={{ flex: 1 }}><Text className="text-sm font-medium text-gray-700 mb-1">State</Text><TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.state} onChangeText={(t) => setFormData({...formData, state: t})} placeholder="State" /></View>
                </View>
              )}
              {view === 'sites' && (
                <>
                  <Text className="text-sm font-medium text-gray-700 mb-1">Classification</Text>
                  <TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.classification} onChangeText={(t) => setFormData({...formData, classification: t})} placeholder="e.g., PRODUCTION" />
                </>
              )}
              {view === 'units' && (
                <>
                  <Text className="text-sm font-medium text-gray-700 mb-1">Unit Type</Text>
                  <TextInput className="border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm mb-3" value={formData.unitType} onChangeText={(t) => setFormData({...formData, unitType: t})} placeholder="e.g., PRODUCTION_LINE" />
                </>
              )}
            </ScrollView>
          </View>
        ...
        <View className="flex-1 bg-black/50 justify-center items-center p-4">
          <View className="bg-white rounded-xl p-6 w-80 shadow-lg">
            ...
            <View className="flex-row justify-end gap-3">
              ...
              <TouchableOpacity onPress={() => { if (deleteConfirm?.id) executeDelete(deleteConfirm.id); }} className="px-4 py-2.5 rounded-lg bg-red-600"><Text className="text-white text-sm font-semibold">Delete</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
