...
export default function RoleManagement() {
  ...
  const [initialFormData, setInitialFormData] = useState<RoleFormData>(emptyFormData);
  ...
  const updateFormField = (field: keyof RoleFormData, value: any) => {
    setFormData(prev => ({ ...prev, [field]: value }));
    if (JSON.stringify({ ...formData, [field]: value }) !== JSON.stringify(initialFormData)) {
      markDirty();
    }
  };
  ...
  const openModal = (role?: Role) => {
    if (role) {
      ...
      const data = {
        ...
        minimumCompetencyLevel: role.minimumCompetencyLevel || 1,
      };
      setFormData(data);
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white px-4 pt-4 pb-3 border-b border-gray-200">
        <View className="flex-row justify-between items-center" style={isDesktop ? { maxWidth: 1200, alignSelf: 'center', width: '100%' } : undefined}>
          ...
          <View className="flex-row gap-2">
            <TouchableOpacity onPress={fetchRoles} className="p-2 bg-gray-100 rounded-lg">
            ...
            >
              ...
              <Text className="text-white ml-1 text-sm font-medium">Add Role</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
        ...
        <ScrollView className="flex-1" contentContainerStyle={isDesktop ? { maxWidth: 1200, alignSelf: 'center', width: '100%', paddingHorizontal: 16, paddingTop: 8 } : { paddingHorizontal: 16, paddingTop: 8 }}>
          {filteredRoles.length === 0 ? (
            <View className="items-center py-16">
              ...
              <Text className="text-gray-400 mt-3 text-base">{searchQuery ? 'No roles match' : 'No roles found'}</Text>
            </View>
          ) : (
            <>
              {/* Mobile: Cards */}
                  ...
                  <View className="px-4 py-3">
                    <View className="flex-row justify-between items-start">
                        ...
                        <View className="flex-row flex-wrap gap-2 mt-2">
                          {role.requiresCompetency && 
                            <View className="bg-purple-100 px-2 py-0.5 rounded-full">
                              <Text className="text-xs text-purple-700">Competency L{role.minimumCompetencyLevel || 1}</Text>
                            </View>
                          }
                          {role.auditorRole && 
                            <View className="bg-blue-100 px-2 py-0.5 rounded-full">
                              <Text className="text-xs text-blue-700">Auditor</Text>
                            </View>
                          }
                        </View>
                        {role.description && 
                          <Text className="text-xs text-gray-400 mt-1" numberOfLines={2}>{role.description}</Text>
                        }
                      </View>
                      <View className="flex-row ml-2">
                        ...
                        <TouchableOpacity 
                          onPress={() => setDeleteConfirm({ id: String(role.id), name: role.displayName })} 
                  ...
                  {filteredRoles.map((role) => (
                    <View key={role.id?.toString()} className="flex-row items-center py-3 px-4 border-b border-gray-100">
                      <View className="flex-1">
                        <Text className="text-sm font-semibold text-gray-900">{role.displayName}</Text>
                        {role.description && 
                          <Text className="text-xs text-gray-400 mt-0.5" numberOfLines={1}>{role.description}</Text>
                        }
                      </View>
                      ...
                      <View className="w-32 flex-row gap-1">
                        ...
                        {role.requiresCompetency && 
                          <View className="bg-purple-100 px-1.5 py-0.5 rounded-full">
                            <Text className="text-xs text-purple-700">L{role.minimumCompetencyLevel || 1}</Text>
                          </View>
                        }
                        {!role.auditorRole && !role.requiresCompetency && 
                          <Text className="text-xs text-gray-400">Standard</Text>
                        }
                      </View>
          ...
          <View style={isDesktop ? { backgroundColor: 'white', borderRadius: 16, width: 500, maxHeight: '80%' } : { backgroundColor: 'white', borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: '80%' }}>
            ...
            <ScrollView className="px-5 py-4" showsVerticalScrollIndicator={false}>
              ...
              <TextInput 
                ...
                value={formData.name} 
                onChangeText={(t) => updateFormField('name', t.toUpperCase())} 
              ...
              <TextInput 
                ...
                onChangeText={(t) => updateFormField('displayName', t)} 
                placeholder="e.g., Supervisor" 
              />
              ...
              <View className="flex-row justify-between items-center py-2 mb-2">
                ...
                <Switch 
                  ...
                  onValueChange={(v) => updateFormField('requiresCompetency', v)} 
                ...
                <View className="mb-3">
                  ...
                  <View className="flex-row gap-2">
                    {[1, 2, 3, 4].map(level => (
                      <TouchableOpacity 
                        key={level} 
                        onPress={() => updateFormField('minimumCompetencyLevel', level)} 
                        className={`flex-1 py-2 rounded-lg border ${formData.minimumCompetencyLevel === level ? 'bg-blue-900 border-blue-900' : 'bg-white border-gray-300'}`}
                      >
                        <Text className={`text-xs text-center font-medium ${formData.minimumCompetencyLevel === level ? 'text-white' : 'text-gray-600'}`}>
                          Level {level}
                        </Text>
                      </TouchableOpacity>
                    ))}
              ...
              <View className="flex-row justify-between items-center py-2 mb-2">
                ...
                <Switch 
                  ...
                  onValueChange={(v) => updateFormField('auditorRole', v)} 
                  trackColor={{ false: '#d1d5db', true: '#00529B' }} 
                />
              </View>
            </ScrollView>
      ...
      {/* ✅ Discard Changes Modal - Same as UserFormModal */}
      <Modal
