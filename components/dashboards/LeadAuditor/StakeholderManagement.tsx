...
const StakeholderManagement: React.FC<StakeholderManagementProps> = ({
  ...
  const [selectedStakeholder, setSelectedStakeholder] = useState<User | null>(null);
  ...
  const renderStakeholderCard = (item: User) => {
    ...
    const assignedAudits = allSchedules.filter(s => 
      activeTab === 'auditors' ? s.auditorId === item.id : s.auditeeId === item.id
    ).length;
    ...
    return (
      <View key={String(item.id)} style={styles.stakeholderCard}>
        <View style={styles.stakeholderHeader}>
          <View style={[styles.avatar, { backgroundColor: NAVBAR_COLORS.primary }]}>
            <Text style={styles.avatarText}>
              {(item.firstName?.[0] || item.username?.[0] || 'A').toUpperCase()}
            </Text>
          </View>
          <View style={styles.stakeholderInfo}>
            <Text style={styles.stakeholderName}>{item.firstName} {item.lastName}</Text>
            <Text style={styles.stakeholderRole}>{item.role || 'User'} • {item.email || 'No email'}</Text>
          </View>
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <Text style={[styles.statBoxValue, { color: NAVBAR_COLORS.primary }]}>{responses.length}</Text>
        ...
        <View style={styles.statusRow}>
          <View style={[styles.statusBox, { backgroundColor: '#D1FAE5' }]}>
            <Text style={[styles.statusBoxValue, { color: '#059669' }]}>{approvedResponses}</Text>
            <Text style={styles.statusBoxLabel}>Approved</Text>
          </View>
          <View style={[styles.statusBox, { backgroundColor: '#FEF3C7' }]}>
            <Text style={[styles.statusBoxValue, { color: '#D97706' }]}>{submittedResponses}</Text>
  ...
  const renderModalItem = ({ item }: { item: Response | NCR }) => {
    if (modalType === 'responses') {
      ...
      return (
        <View style={styles.modalItem}>
          <View style={styles.modalItemHeader}>
            <Text style={styles.modalItemId}>{answers?.documentNumber || `RES-${r.id}`}</Text>
            <View style={[styles.modalItemBadge, { backgroundColor: statusColors.bg }]}>
              <Text style={[styles.modalItemBadgeText, { color: statusColors.text }]}>
                {r.status || 'DRAFT'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.modalItemFooter}>
            <Text style={styles.modalItemScore}>
              Score: <Text style={styles.modalItemScoreValue}>{(r.percentageScore || 0).toFixed(1)}%</Text>
            </Text>
            <TouchableOpacity 
      ...
      return (
        <View style={styles.modalItem}>
          <View style={styles.modalItemHeader}>
            <View style={[styles.modalItemBadge, { backgroundColor: severityColors.bg }]}>
              <Text style={[styles.modalItemBadgeText, { color: severityColors.text }]}>
                {n.severity || 'NCR'}
              </Text>
            </View>
            <View style={[styles.modalItemBadge, { backgroundColor: statusColors.bg }]}>
          ...
          <Text style={styles.modalItemTitle}>{n.ncrNumber || `NCR-${n.id}`}</Text>
          <View style={styles.modalItemFooter}>
            <Text style={styles.modalItemSubtext}>Dept: {n.department || 'N/A'}</Text>
            <TouchableOpacity 
              style={styles.modalItemViewButton}
              onPress={() => {
  ...
  const stakeholders = activeTab === 'auditors' 
    ? allAuditors.filter(a => a.role !== 'LEAD_AUDITOR' && !a.role?.toLowerCase().includes('lead'))
    : allAuditees;

  const selectedResponses = selectedStakeholder ? getResponses(selectedStakeholder.id) : [];
  const selectedNCRs = selectedStakeholder ? getNCRs(selectedStakeholder.id) : [];
      ...
      <View style={styles.grid}>
        {stakeholders.length === 0 ? (
          <View style={styles.emptyState}>
            <Icon name={activeTab === 'auditors' ? 'users' : 'user-check'} size={40} color="#CBD5E1" />
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, isMobile && styles.modalContentMobile]}>
            <View style={[styles.modalHeader, { backgroundColor: NAVBAR_COLORS.primary }]}>
              <View>
                <Text style={styles.modalHeaderTitle}>
                  Responses by {selectedStakeholder?.firstName} {selectedStakeholder?.lastName}
                </Text>
            ...
            {selectedStakeholder && (
              <View style={styles.modalStatsGrid}>
                <View style={styles.modalStat}>
                  <Text style={styles.modalStatValue}>{getSummary(selectedStakeholder.id).total}</Text>
                  <Text style={styles.modalStatLabel}>Total</Text>
                </View>
                <View style={styles.modalStat}>
                  <Text style={[styles.modalStatValue, { color: '#059669' }]}>{getSummary(selectedStakeholder.id).approved}</Text>
                  <Text style={styles.modalStatLabel}>APPROVED</Text>
                </View>
                <View style={styles.modalStat}>
                  <Text style={[styles.modalStatValue, { color: '#DC2626' }]}>{getSummary(selectedStakeholder.id).rejected}</Text>
                  <Text style={styles.modalStatLabel}>REJECTED</Text>
                </View>
                <View style={styles.modalStat}>
                  <Text style={[styles.modalStatValue, { color: '#D97706' }]}>{getSummary(selectedStakeholder.id).pending}</Text>
                  <Text style={styles.modalStatLabel}>SUBMITTED</Text>
                </View>
                <View style={styles.modalStat}>
                  <Text style={[styles.modalStatValue, { color: NAVBAR_COLORS.primary }]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, isMobile && styles.modalContentMobile]}>
            <View style={[styles.modalHeader, { backgroundColor: NAVBAR_COLORS.primary }]}>
              <View>
                <Text style={styles.modalHeaderTitle}>
                  NCRs by {selectedStakeholder?.firstName} {selectedStakeholder?.lastName}
                </Text>
                <Text style={styles.modalHeaderSubtext}>
