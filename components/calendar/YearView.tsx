...
export default function YearView({
  ...
  const getEventsForMonth = (monthIndex: number) => {
    ...
    return validEvents.filter((e: any) =>
      moment(e.start).isBetween(start, end, "day", "[]"),
    );
  };
  ...
  const getEventColor = (event: any) => {
    ...
    if (event.status === "OVERDUE" || event.status === "REJECTED")
      return "#EF4444";
  ...
  const renderMiniMonth = (monthIndex: number) => {
    ...
    while (
      currentWeek.isSameOrBefore(monthEnd, "day") ||
      currentWeek.month() === monthIndex
    ) {
      const week: any[] = [];
      for (let i = 0; i < 7; i++) {
        const day = currentWeek.clone().add(i, "day");
        const dayEvents = validEvents.filter((e: any) =>
      ...
      <View style={{ marginTop: 4 }}>
        ...
        {weeks.map((week, wIdx) => (
          <View key={wIdx} style={{ flexDirection: "row", marginBottom: 4 }}>
            {week.map((d: any, dIdx: number) => (
              <TouchableOpacity
                key={dIdx}
                disabled={!d.isCurrentMonth}
                onPress={() =>
                  d.isCurrentMonth && onDateClick && onDateClick(d.date)
                }
                style={{
                  flex: 1,
                  height: 34,
                  ...
                  backgroundColor: d.isToday
                    ...
                    : d.hasEvents
                      ? "#EFF6FF"
                      : "transparent",
                  borderWidth: d.hasEvents && !d.isToday ? 1 : 0,
                  borderColor: "#BFDBFE",
                }}
              >
                {/* ✅ STATUS BARS AT TOP (like web image) */}
                {d.hasEvents && (
                  <View
                    style={{
                      position: "absolute",
                    ...
                    {d.colors.map((c: string, i: number) => (
                      <View
                        ...
                        style={{
                          ...
                          height: 3,
                          borderRadius: 2,
                          backgroundColor: d.isToday ? "#FFFFFF" : c,
                        }}
                      />
                    ))}
                  </View>
  ...
  const completedEvents = validEvents.filter(
    (e: any) => e.isFullyCompleted,
  ).length;
  const pendingEvents = validEvents.filter(
                ...
                >
                  <TouchableOpacity
                    onPress={() =>
                      onMonthClick &&
                      onMonthClick(
                        moment().year(currentYear).month(monthIndex).toDate(),
                      )
                    }
                  >
                    <Text
